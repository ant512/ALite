...
namespace ALite
{
	...
	public class StringLengthValidationRule : ValidationRule
	{
		...
		public override bool Validate(object value, ref string errorMessage)
		{
			...
			{
				errorMessage = String.Format("String is longer than maximum length of {0} chars.", mMaxLength);
				return false;
			}

			if (!ValidateMinLength((string)value))
			{
				errorMessage = String.Format("String is shorter than minimum length of {0} chars.", mMinLength);
				return false;
			}
